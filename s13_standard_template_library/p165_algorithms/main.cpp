...
void UserDefined() {
	...
	// using std::count_if with a lambda expression for the predicate callback is shorter and more meaningful
	// to the reader upon first inspectiona
	...
	// for_each is basically a for loop with a callback function
	// Get ids of all employees that know C++
	// for_each returns instance of functor
	auto foundIds = std::for_each(v.begin(), v.end(), EmpIds());
	// print the IDs returned by for_each
	for (int id : foundIds.m_Ids) {
		std::cout << "Employees that know c++ Ids:" << id << std::endl;
	}
