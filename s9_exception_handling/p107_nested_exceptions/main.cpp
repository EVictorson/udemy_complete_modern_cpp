...
void ProcessRecords(int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        try {
            ...
            // if there are more than 4 errors assign the new runtime_error to ex and throw it up to the main()
            // inner throw block re-throws exception it cannot handle up a level
            if (errors > 4) {
